...
namespace MyGUI
{
	...
	void FontManager::_load(xml::ElementPtr _node, const std::string & _file, Version _version)
	{
		...
		while (font.next(XML_TYPE)) {
			...
			std::string source, name, size, resolution, antialias, space, tab, distance, cursor, offsetH, shadow;
			...
			font->findAttribute("shadow", shadow);
			...
			if (false == shadow.empty()) pFont->setShadow(utility::parseBool(shadow));
