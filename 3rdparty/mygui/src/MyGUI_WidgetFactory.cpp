...
namespace MyGUI
{
	namespace factory
	{
			...
			mNeedTranslate(false)
		{
			...
			manager.registerDelegate("Widget_Shadow") = newDelegate(this, &WidgetFactory::Widget_Shadow);
		...
		WidgetFactory::~WidgetFactory()
		{
			...
			manager.unregisterDelegate("Widget_Shadow");
		...
		void WidgetFactory::Widget_Shadow(WidgetPtr _widget, const std::string &_key, const std::string &_value)
		{
			_widget->setShadow(utility::parseBool(_value));
		}
