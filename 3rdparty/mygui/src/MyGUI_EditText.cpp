...
namespace MyGUI
{
	...
	void EditText::setShadow(bool enabled)
	{
		if (mShadowEnabled == enabled) return;
	...
	{
		return _drawItemEx(_vertex, _update, false);
	}

	size_t EditText::_drawItemEx(Vertex * _vertex, bool _update, bool _shadow)
	{
		...
		uint32 colour_shadow = Colour(0.25, 0.28, 0.29).toColourARGB();
		...
		float real_left_s = ((mManager->getPixScaleX() * (float)(mCurrentCoord.left + mCroppedParent->getAbsoluteLeft() + 1) + mManager->getHOffset()) * 2) - 1;
		float real_top_s = -(((mManager->getPixScaleY() * (float)(mCurrentCoord.top + mCroppedParent->getAbsoluteTop() + (mShiftText ? 1 : 0) + 1) + mManager->getVOffset()) * 2) - 1);
		float real_width_s = (mManager->getPixScaleX() * (float)mCurrentCoord.width * 2);
		float real_height_s = (mManager->getPixScaleY() * (float)mCurrentCoord.height * 2);
		float real_right_s = real_left_s + real_width_s;
		float real_bottom_s = real_top_s - real_height_s;
		...
		float margin_left_s = (mMargin.left * mManager->getPixScaleX() * 2);
		float margin_right_s = (mMargin.right * mManager->getPixScaleX() * 2);
		float margin_top_s = (mMargin.top * mManager->getPixScaleY() * 2);
		float margin_bottom_s = (mMargin.bottom * mManager->getPixScaleY() * 2);

		// опорное смещение вершин
		float left, right, top, bottom = real_top, left_shift = 0;
		float left_s, right_s, top_s, bottom_s = real_top_s, left_shift_s = 0;
		...
		if (false == mManualView) {
			if ( mTextAlign.isRight() ) {
				// выравнивание по правой стороне
				left_shift = mContextRealSize.width - real_width;
				left_shift_s = mContextRealSize.width - real_width;
			}
			else if ( mTextAlign.isHCenter() ) {
				// для середины нужно четное число
				float tmp = ((mCoord.width - mContextSize.width) >> 1) << 1;
				left_shift = -(mManager->getPixScaleX() * (float)(tmp));
				left_shift_s = -(mManager->getPixScaleX() * (float)(tmp));
			}
		}
		else {
			left_shift = (mManager->getPixScaleX() * (float)mViewOffset.left * 2.0);
			left_shift_s = (mManager->getPixScaleX() * (float)mViewOffset.left * 2.0);
		}

		if (false == mManualView) {
			if ( mTextAlign.isTop() ) 	{
				bottom += margin_top;
				bottom_s += margin_top_s;
			}
			else if ( mTextAlign.isBottom() ) {
				bottom += mContextRealSize.height - real_height - margin_bottom;
				bottom_s += mContextRealSize.height - real_height_s - margin_bottom_s;
			}
			else if ( mTextAlign.isVCenter() ) {
				float tmp = ((mCoord.height - mContextSize.height) >> 1) << 1; // для середины нужно четное число
				bottom += margin_top - (mManager->getPixScaleY() * (float)(tmp));
				bottom_s += margin_top_s - (mManager->getPixScaleY() * (float)(tmp));
			}
		}
		else {
			bottom = real_top + margin_top + (mManager->getPixScaleY() * (float)mViewOffset.top * 2.0);
			bottom_s = real_top_s + margin_top_s + (mManager->getPixScaleY() * (float)mViewOffset.top * 2.0);
		}

		// данные непосредственно для вывода
		float vertex_top, vertex_bottom, vertex_left, vertex_right;
		float vertex_top_s, vertex_bottom_s, vertex_left_s, vertex_right_s;
		...
		for (VectorLineInfo::iterator line = mLinesInfo.begin(); line != end; ++line) {
			...
			if (vertex_top_s > real_top_s) {//********************************
				// проверка на полный выход
				vertex_top_s = real_top_s;
				texture_crop_height = true;
			}
			if (vertex_bottom_s < real_bottom_s) {
				// обрезаем
				vertex_bottom_s = real_bottom_s;
				texture_crop_height = true;
			}//**********************************
			...
			if ( mTextAlign.isLeft() ) {
				// выравнивание по левой стороне
				right = real_left - left_shift - margin_left;
				right_s = real_left_s - left_shift_s - margin_left_s;
			}
			else if ( mTextAlign.isRight() ) {
				// выравнивание по правой стороне
				right = real_left - left_shift + (mContextRealSize.width - info.real_length) + margin_right;
				right_s = real_left_s - left_shift_s + (mContextRealSize.width - info.real_length) + margin_right_s;
			}
			else {
				// выравнивание по центру
				int tmp = ((mContextSize.width - info.length) >> 1) << 1; // для середины нужно четное число
				right = real_left - left_shift + (((mManager->getPixScaleX() * (float)tmp * 2.0)) * 0.5) - margin_left;
				right_s = real_left_s - left_shift_s + (((mManager->getPixScaleX() * (float)tmp * 2.0)) * 0.5) - margin_left_s;
			}
			...
			for (;index != end_index; ++index) {
				...
				vertex_left_s = left_s;
				vertex_right_s = right_s;
				...
				float texture_left_s = info->uvRect.left;
				float texture_right_s = info->uvRect.right;
				float texture_top_s = info->uvRect.top;
				float texture_bottom_s = info->uvRect.bottom;
				...
				if (vertex_left_s < real_left_s) {//*******************************************
					vertex_left_s = real_left_s;
					texture_crop_width = true;
				}
				if (vertex_right_s > real_right_s) {
					// обрезаем
					vertex_right_s = real_right_s;
					texture_crop_width = true;
				}//**********************************
				...
				if (texture_crop_height) {
					// прибавляем размер смещения в текстурных координатах
					texture_top_s += (top_s - vertex_top_s) * mTextureHeightOne;
					// отнимаем размер смещения в текстурных координатах
					texture_bottom_s -= (vertex_bottom_s - bottom_s) * mTextureHeightOne;
				}//****************************************
				...
				if (texture_crop_width) {//************************************
					// прибавляем размер смещения в текстурных координатах
					texture_left_s += (vertex_left_s - left_s) * mTextureWidthOne;
					// отнимаем размер смещения в текстурных координатах
					texture_right_s -= (right_s - vertex_right_s) * mTextureWidthOne;
				}//**************************************************
				...
				if(mShadowEnabled)
				{
					__MYGUI_DRAW_QUAD(_vertex, vertex_left_s, vertex_top_s, vertex_right_s, vertex_bottom_s, vertex_z, colour_shadow,
						texture_left_s, texture_top_s, texture_right_s, texture_bottom_s, vertex_count);
				}
